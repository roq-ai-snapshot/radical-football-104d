...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['academy-owner', 'coach', 'player'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AcademyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/academies` : null),
    () => getAcademies({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Academy',
      path: isTenantUser ? `/academies/view/${data?.[0]?.id}` : '/academies',
      entity: 'academy',
    ...
    { name: 'Player', path: '/players', entity: 'player', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
    {
