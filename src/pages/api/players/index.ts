...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createPlayer() {
    ...
    if (body?.training_session?.length > 0) {
      const create_training_session = body.training_session;
      body.training_session = {
        create: create_training_session,
      };
    } else {
      delete body.training_session;
    }
